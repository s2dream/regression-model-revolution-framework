...
* **REQ-EX-03: Optuna 기반 자동 하이퍼파라미터 튜닝 (Auto HPO)**
  - **설명**: 설정 파일의 `hpo.enabled`가 `True`일 때 학습 단계 이전에 Optuna 프레임워크를 기반으로 하이퍼파라미터 자동 튜닝을 구동하여 검증 RMSE를 최소화하는 최적의 파라미터를 탐색해 적용해야 합니다. 튜닝이 무의미하거나 불가능한 모델(예: TabPFN)이나 커스텀 모델은 안전하게 스킵되어야 합니다.
...
* **REQ-UI-05: HPO 튜닝 활성화 및 최적화 시도 횟수 지정 UI 지원**
  - **설명**: 웹 대시보드 사이드바(Sidebar) 영역에서 사용자가 HPO 기능 기동 여부와 탐색 시도 횟수(`n_trials`)를 명시적으로 조작하고, 이를 `web_config.yml` 컴파일 구조에 병합하여 실행 파이프라인에 전달할 수 있게 구성해야 합니다.
